...
| 가치 | 설명 |
|------|------|
| **디지털 자아 보존** | 나의 생각, 감정, 성격이 디지털로 남음 |
| **시간을 초월한 대화** | 과거의 나, 미래의 나와 대화 |
| **관계의 디지털 유산** | 소중한 사람의 "본질"을 보존 |
| **비동기 공감** | 직접 만나지 않아도 상대를 이해 |

### 타겟 사용자

#### 세그먼트별 정의

| 순위 | 세그먼트 | 연령 | 특성 | 마케팅 전략 |
|------|----------|------|------|------------|
| **1차** | 자기 성찰 추구자 | 25-35세 | MBTI/성격유형 관심, 내면 탐구 욕구 | 적극 마케팅 |
| **2차** | 관계 깊이 추구자 | 20-30세 | 친밀한 관계 중시, 친구/연인과의 소통 | 적극 마케팅 |
| **3차** | 디지털 레거시 관심자 | 30-50세 | 자녀 있음, 기록/유산에 관심 | 신중하게 접근 |
| **4차** | 추모/기억 보존자 | 전 연령 | 소중한 사람의 기억 보존 니즈 | 직접 마케팅 X |
...
| ID | 기능명 | 설명 | 상태 |
|----|--------|------|------|
| F16 | **멘탈 상태 분석** | 일기 기반 AI 심리 분석 (정서 안정, 활력, 자존감, 긍정성, 사회적 연결, 회복탄력성) | ✅ 완료 |
| F17 | **심리 인바디 체크** | 6축 레이더 차트로 멘탈 상태 시각화 | ✅ 완료 |
| F18 | **감정 기반 피드백** | 멘탈 상태별 차등 응원 메시지 (가벼운 응원 ~ 심리 상담 제안) | ✅ 완료 |
| F19 | **책 추천 기능** | 현재 감정에 맞는 책 자동 추천 | ✅ 완료 |
| F20 | **주간/월간 리포트** | 멘탈 변화 추이 및 인사이트 제공 | ✅ 완료 |

### Phase 5 - 온보딩 & 구독 (완료)

| ID | 기능명 | 설명 | 상태 |
|----|--------|------|------|
| F21 | **성격 퀴즈** | 일기 없이도 퀴즈로 임시 페르소나 생성 | ✅ 완료 |
| F22 | **페르소나 레벨 시스템** | 임시 → 기본 → 성장 레벨업 | ✅ 완료 |
| F23 | **프리미엄 구독** | 월정액 구독 시스템 (테스트) | ✅ 완료 |
| F24 | **법적 페이지** | 개인정보처리방침, 이용약관 | ✅ 완료 |
...
| F26 | 결제 연동 | 실제 결제 시스템 연동 |
| F27 | 멀티모달 캐릭터 | DALL-E 3 기반 캐릭터 이미지 생성 |
...
| 마일스톤 | 작업 내용 | 상태 |
|----------|----------|------|
| M11 | 멘탈 분석 시스템 (DB, API, 서비스) | ✅ 완료 |
| M12 | 심리 인바디 레이더 차트 | ✅ 완료 |
| M13 | 감정 기반 피드백 시스템 | ✅ 완료 |
| M14 | 책 추천 기능 | ✅ 완료 |
| M15 | 주간/월간 리포트 | ✅ 완료 |
| M16 | 일기 작성 시 자동 분석 통합 | ✅ 완료 |
...
| M17 | 성격 퀴즈 시스템 | ✅ 완료 |
| M18 | 페르소나 레벨 시스템 | ✅ 완료 |
| M19 | 프리미엄 구독 시스템 (테스트) | ✅ 완료 |
| M20 | 법적 페이지 (개인정보처리방침, 이용약관) | ✅ 완료 |
...
| POST | `/api/v1/quiz/submit` | 퀴즈 제출 및 임시 페르소나 생성 (`{answers[]}`) |
| GET | `/api/v1/quiz/persona-level` | 페르소나 레벨 정보 조회 |
...
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/api/v1/mental/current` | 최근 멘탈 상태 조회 (피드백 포함) |
| GET | `/api/v1/mental/radar` | 레이더 차트 데이터 조회 |
| GET | `/api/v1/mental/history` | 분석 이력 조회 (Query: `?days=&skip=&limit=`) |
| GET | `/api/v1/mental/reports/weekly` | 주간 리포트 |
| GET | `/api/v1/mental/reports/monthly` | 월간 리포트 |
| POST | `/api/v1/mental/feedback` | 피드백 생성 (`{analysis_id?}`) |
| GET | `/api/v1/mental/book-recommendations` | 책 추천 |
| GET | `/api/v1/mental/check-negative-trend` | 부정적 추세 확인 |
...
| GET | `/api/v1/subscriptions/status` | 구독 상태 간단 조회 |
| GET | `/api/v1/subscriptions/plans` | 이용 가능 플랜 목록 |
...
| `/diaries/:id` | 일기 상세/수정 |
| `/persona` | 내 페르소나 프로필 |
| `/persona/chat` | 내 페르소나와 대화 |
| `/persona/chat/:chatId` | 특정 대화 |
| `/friends` | 친구 목록 |
| `/notifications` | 알림 목록 |
| `/mental` | 멘탈 대시보드 (레이더 차트) |
| `/mental/books` | 책 추천 |
| `/mental/reports` | 주간/월간 리포트 |
| `/premium` | 프리미엄 구독 |
| `/privacy` | 개인정보처리방침 |
| `/terms` | 이용약관 |
