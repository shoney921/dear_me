...
def send_password_reset_email(user: User, token: str) -> bool:
    ...
    html_content = _build_password_reset_html(user.username, reset_url)

    msg = MIMEMultipart("alternative")
    msg["Subject"] = "[DearMe] 비밀번호 초기화 안내"
    msg["From"] = f"{settings.SMTP_FROM_NAME} <{settings.SMTP_USER}>"
    msg["To"] = user.email
...
def invalidate_password_reset_token(user: User, db: Session) -> None:
...
def _build_password_reset_html(username: str, reset_url: str) -> str:
    return f"""<!DOCTYPE html>
<html lang="ko">
<head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background-color:#f4f4f7;font-family:'Apple SD Gothic Neo','Malgun Gothic',sans-serif;">
      ...
      <td align="center">
        <table width="480" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#6366f1,#8b5cf6);padding:32px;text-align:center;">
              <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;">DearMe</h1>
              <p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">비밀번호 초기화</p>
            </td>
          </tr>
          <!-- Body -->
          <tr>
